...
export const readArrayBufferAsUtf8 = (memory: WebAssembly.Memory, ptr: number) => {
    return Buffer.from(Array.from(new Uint8Array(readArrayBuffer(memory, ptr)))).toString('utf8');
};

export const readArrayBufferAsHex = (memory: WebAssembly.Memory, ptr: number) => {
    return (
        '0x' + Buffer.from(Array.from(new Uint8Array(readArrayBuffer(memory, ptr)))).toString('hex')
    );
};
...
export const readTypedArray = (memory: WebAssembly.Memory, ptr: number) => {
    ...
    return new Uint8Array(
        new Buffer(new Uint8Array(memory.buffer)).slice(dataStart, dataStart + realLength),
    ).buffer;
};
